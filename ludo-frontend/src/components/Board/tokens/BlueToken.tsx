...
const BlueToken: React.FC<BlueTokenProps> = ({
  size = 32,
  className = '',
  isAnimated = false,
  isGlowing = false,
  ...
  const tokenId = `blue-token-${Math.random().toString(36).substr(2, 9)}`;
  ...
  return (
    <svg
      ...
      height={size}
      viewBox="0 0 64 64"
      className={`ludo-token blue-token ${className} ${isAnimated ? 'animated' : ''} ${isGlowing ? 'glowing' : ''}`}
      onClick={onClick}
